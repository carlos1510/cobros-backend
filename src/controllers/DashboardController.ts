    ...
    public async getReporteGeneral(req: Request, res: Response){
        try {
            ...
            const resulCredito = await sequelize.query(
                ...
                WHERE cd.state = 1 AND cp.id=${companie?.id} AND cd.creditDate BETWEEN '${formatoFecha(fecha_inicio as string)}' AND '${formatoFecha(fecha_fin as string)}'`,
            ...
            const resulTotalCobro = await sequelize.query(
                ...
                inner join users u ON u.id = cd.userId 
                inner join companies cp ON u.id = cp.userId
                WHERE cd.state IN (1,2) AND f.state=1 AND cp.id=${companie?.id} AND f.payDate BETWEEN '${formatoFecha(fecha_inicio as string)}' AND '${formatoFecha(fecha_fin as string)}'`,
                { type: QueryTypes.SELECT }
            );
            ...
            const resultsClients = await sequelize.query(
                ...
                DATE_FORMAT(cd.endDate,'%d/%m/%Y') AS finishedDate
                                ...
                                inner join users u ON u.id = cd.userId 
                                inner join companies cp ON u.id = cp.userId
                                WHERE cd.state = 1 AND cp.id=${companie?.id} AND cd.endDate BETWEEN '${formatoFecha(fecha_inicio as string)}' AND '${formatoFecha(fecha_fin as string)}'
                ) AS tb `,
                { type: QueryTypes.SELECT }
            )
            res.status(200).json({
